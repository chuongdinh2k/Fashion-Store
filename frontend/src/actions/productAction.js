...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${process.env.REACT_APP_API_KEY}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;

      if (category) {
        link = `${process.env.REACT_APP_API_KEY}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
      }
...
export const createProduct = (productData, token) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API_KEY}/api/v1/admin/product/new`,
      productData,
      config
    );
